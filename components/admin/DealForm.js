...
export default function DealForm({ deal, onClose, onSaved }) {
  ...
  const handleImageUpload = async (e) => {
    ...
    if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_UPLOAD_PRESET) {
      setError('Cloudinary not configured. Add NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME and NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET to environment variables.');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.title || !form.sale_price || !form.affiliate_link) {
      setError('Title, Sale Price, and Affiliate Link are required.');
      setSaving(false);
      return;
    }
    ...
    let result;
    if (isEdit) {
      result = await supabase.from('deals').update(payload).eq('id', deal.id);
    } else {
